...
class ThriftVisitor(ThriftGrammerVisitor):
    ...
    def visitField_type(
        ...
        if ctx.container_type():
            return self.visit(ctx.container_type())
        elif ctx.base_type():
            return self.visit(ctx.base_type())
        elif ctx.IDENTIFIER():
            return NullType()
        else:
            raise NotImplementedError()
    ...
    def visitContainer_type(
        self, ctx: ThriftGrammerParser.Container_typeContext
    ) -> SchemaFieldDataType:
        if ctx.list_type():
    ...
    def visitList_type(
        self, ctx: ThriftGrammerParser.List_typeContext
    ) -> SchemaFieldDataType:
        return SchemaFieldDataType(ArrayType([ctx.field_type().getText()]))  # TODO: hack
